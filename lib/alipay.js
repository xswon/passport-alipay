...
Alipay.prototype.getAccessToken = function (code, cb) {
    ...
    rp({
        ...
        if (response.error_response) {
            cb(new Error(response.error_response.msg));
        } else if (!response.alipay_system_oauth_token_response) {
            cb(new Error('Invalid Arguments'));
        } else {
            cb(null, response.alipay_system_oauth_token_response);
        }
    }).catch(function (err) {
        cb(err);
    });
...
Alipay.prototype.getUser = function (token, cb) {
    ...
    rp({
        ...
        if (response.error_response) {
            cb(new Error(response.error_response.msg));
        } else if (!response.alipay_user_info_share_response) {
            cb(new Error('Invalid Arguments'));
        } else {
            cb(null, response.alipay_user_info_share_response);
        }
    }).catch(function (err) {
        cb(err);
    });
